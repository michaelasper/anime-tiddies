...
struct RenderInputMeta {
    ...
    size_t getElementSize()
        const;  // simple check: return 12 (3 * 4 bytes) for float3
    RenderInputMeta();
    RenderInputMeta(int _position, const std::string& _name, const void* _data,
                    size_t _nelements, size_t _element_length,
                    int _element_type);
...
RenderInputMeta::RenderInputMeta(int _position, const std::string& _name,
                                 const void* _data, size_t _nelements,
                                 size_t _element_length, int _element_type)
    : position(_position),
      name(_name),
      data(_data),
...
RenderDataInput::RenderDataInput() : index_meta_(new RenderInputMeta) {}
...
RenderPass::RenderPass(
    ...
    if (shaders[1]) glAttachShader(sp_, gs_);

    // ... and then buffers
    size_t nbuffer = input.getNBuffers();
    if (input.hasIndex()) nbuffer++;
    ...
    for (int i = 0; i < input.getNBuffers(); i++) {
        ...
        if (meta.isInteger()) {
            CHECK_GL_ERROR(glVertexAttribIPointer(
                meta.position, meta.element_length, meta.element_type, 0, 0));
        } else {
            CHECK_GL_ERROR(
                glVertexAttribPointer(meta.position, meta.element_length,
                                      meta.element_type, GL_FALSE, 0, 0));
        }
        CHECK_GL_ERROR(glEnableVertexAttribArray(meta.position));
        // ... because we need program to bind location
        CHECK_GL_ERROR(
            glBindAttribLocation(sp_, meta.position, meta.name.c_str()));
    }
    ...
    if (input.hasIndex()) {
        ...
        CHECK_GL_ERROR(
            glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, glbuffers_.back()));
    ...
    for (size_t i = 0; i < uniforms.size(); i++) {
        CHECK_GL_ERROR(
            unilocs_[i] = glGetUniformLocation(sp_, uniforms[i]->name.c_str()));
        // std::cerr << "Uniform " << uniforms[i]->name << " has location " <<
        // unilocs_[i] << std::endl;
    }
...
void RenderPass::initMaterialUniform() {
    ...
    for (size_t i = 0; i < input_.getNMaterials(); i++) {
        ...
        V4F diffuse_data = [&ma]() { return ma.diffuse; };
        V4F ambient_data = [&ma]() { return ma.ambient; };
        V4F specular_data = [&ma]() { return ma.specular; };
        FF shininess_data = [&ma]() { return ma.shininess; };
        int texid = matexids_[i];
        int sam = sampler2d_;
        IF texture_data = [texid]() { return texid; };
        IF sampler_data = [sam]() { return sam; };
        ...
        auto texture =
            make_texture("textureSampler", sampler_data, 0, texture_data);
        std::vector<ShaderUniformPtr> munis = {diffuse, ambient, specular,
                                               shininess, texture};
    ...
    CHECK_GL_ERROR(
        malocs_.emplace_back(glGetUniformLocation(sp_, "shininess")));
    CHECK_GL_ERROR(
        malocs_.emplace_back(glGetUniformLocation(sp_, "textureSampler")));
...
void RenderPass::createMaterialTexture() {
    ...
    for (size_t i = 0; i < input_.getNMaterials(); i++) {
        ...
        CHECK_GL_ERROR(glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA8, w, h));
        CHECK_GL_ERROR(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w, h, GL_RGBA,
                                       GL_UNSIGNED_BYTE, dummy.data()));
    ...
    CHECK_GL_ERROR(
        glSamplerParameteri(sampler2d_, GL_TEXTURE_WRAP_S, GL_REPEAT));
    CHECK_GL_ERROR(
        glSamplerParameteri(sampler2d_, GL_TEXTURE_WRAP_T, GL_REPEAT));
    CHECK_GL_ERROR(
        glSamplerParameteri(sampler2d_, GL_TEXTURE_MAG_FILTER, GL_LINEAR));
    CHECK_GL_ERROR(
        glSamplerParameteri(sampler2d_, GL_TEXTURE_MIN_FILTER, GL_LINEAR));
}
...
void RenderPass::updateVBO(int position, const void* data, size_t size) {
    ...
    if (bufferid < 0)
        throw __func__ +
            std::string(": error, can't find buffer with position ") +
            std::to_string(position);
    auto meta = input_.getBufferMeta(bufferid);
    CHECK_GL_ERROR(glBindBuffer(GL_ARRAY_BUFFER, glbuffers_[bufferid]));
    CHECK_GL_ERROR(glBufferData(GL_ARRAY_BUFFER, size * meta.getElementSize(),
                                data, GL_STATIC_DRAW));
}
...
bool RenderPass::renderWithMaterial(int mid) {
    if (mid >= int(material_uniforms_.size()) || mid < 0) return false;
    ...
    CHECK_GL_ERROR(
        glDrawElements(GL_TRIANGLES, mat.nfaces * 3, GL_UNSIGNED_INT,
...
void RenderDataInput::assign(int position, const std::string& name,
                             const void* data, size_t nelements,
                             size_t element_length, int element_type) {
    meta_.emplace_back(position, name, data, nelements, element_length,
                       element_type);
}

void RenderDataInput::assignIndex(const void* data, size_t nelements,
                                  size_t element_length) {
    has_index_ = true;
    *index_meta_ = {-1, "", data, nelements, element_length, GL_UNSIGNED_INT};
}

int RenderDataInput::getNBuffers() const { return int(meta_.size()); }

void RenderDataInput::useMaterials(const std::vector<Material>& ms) {
    materials_ = ms;
    for (const auto& ma : ms) {
        std::cerr << "Use Material from " << ma.offset << " size: " << ma.nfaces
                  << std::endl;
    }
}

const RenderInputMeta& RenderDataInput::getBufferMeta(int i) const {
    return meta_[i];
}
