...
class RenderDataInput {
   public:
     ...
     */
    void assign(int position, const std::string& name, const void* data,
                size_t nelements, size_t element_length, int element_type);
    /*
...
class RenderPass {
   public:
     ...
     *      shaders: array of shaders, leave the second as nullptr if no GS
     * present uniforms: array of ShaderUniform objects output: the FS output
     * variable name. RenderPass does not support render-to-texture or
     * multi-target rendering for now (and you also don't need it).
     */
