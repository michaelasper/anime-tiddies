...
void bindUniform(unsigned loc, int scalar) { glUniform1i(loc, scalar); }

void bindUniform(unsigned loc, float scalar) { glUniform1f(loc, scalar); }
...
void bindUniform(unsigned loc, const std::vector<glm::mat4>& array) {
    glUniformMatrix4fv(loc, array.size(), GL_FALSE,
                       (const GLfloat*)array.data());
}
